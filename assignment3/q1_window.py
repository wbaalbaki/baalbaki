...
from tensorflow.contrib.layers.python.layers.layers import relu
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        fullSent = [start]*window_size + sentence + [end]*window_size

        for i in range(len(sentence)):
            features = []
            for feature in fullSent[i:i+2*window_size+1]:
                features += feature
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        nonConcatenatedEmbeddings =  tf.nn.embedding_lookup(embeddingTensor, self.input_placeholder)
        embeddings = tf.reshape(nonConcatenatedEmbeddings, [-1, self.config.n_window_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, name="W", shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size))
        U = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, name="U", shape=(self.config.hidden_size, self.config.n_classes))
        b1 = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, shape=self.config.hidden_size, name="b1")
        b2 = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, shape=self.config.n_classes, name="b2")
