...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if currLen > max_length:
            ...
            ret.append((sentence + [zero_vector]*(max_length-currLen),
                       labels + [zero_label]*(max_length-currLen),
                       [True]*currLen + [False]*(max_length-currLen)))
        #pass
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(shape=(None, self.max_length, self.config.n_features), name="Input", dtype=tf.int32)
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(nonConcatenatedEmbeddings, [-1, self.config.max_length, self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h_t = tf.zeros(tf.shape(x[:,0,:]),dtype=tf.float32, name='h_t')
        #h_t = tf.zeros(dtype=tf.float32, shape=(None, self.config.hidden_size), name="h_t")
        ### END YOUR CODE
