...
#class RNNCell(tf.contrib.rnn.core_rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable("W_x", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            W_h = tf.get_variable("W_h", shape=(self._state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b = tf.get_variable("b",  initializer=np.array(np.zeros(self._state_size), dtype=np.float32))
