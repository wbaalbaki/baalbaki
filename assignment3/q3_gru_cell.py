...
#class GRUCell(tf.contrib.rnn.core_rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable("W_z", shape=(self._state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_z = tf.get_variable("U_z", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_z = tf.get_variable("b_z",  initializer=np.array(np.zeros(self._state_size), dtype=np.float32))

            W_r = tf.get_variable("W_r", shape=(self._state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_r = tf.get_variable("U_r", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_r = tf.get_variable("b_r",  initializer=np.array(np.zeros(self._state_size), dtype=np.float32))

            W_o = tf.get_variable("W_o", shape=(self._state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_o = tf.get_variable("U_o", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_o = tf.get_variable("b_o",  initializer=np.array(np.zeros(self._state_size), dtype=np.float32))
