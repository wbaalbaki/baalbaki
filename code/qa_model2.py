...
#class GRUAttnCell(tf.rnn_cell.GRUCell):
...
#                ht = tf.rnn_cell._linear(gru_out, self._num_units, True, 1.0)
...
#                out = tf.nn.relu(tf.rnn_cell._linear([context, gru_out], self._num_units, True, 1.0))
#        return (out, out)
...
class Encoder(object):
    def __init__(self, size, vocab_dim):
        ...
        self.cell = tf.nn.rnn_cell.BasicLSTMCell(self.size)#, state_is_tuple=False)
        self.finalCell = tf.nn.rnn_cell.BasicLSTMCell(self.size*4)  # , state_is_tuple=False)
    ...
    # output of an LSTM
    def BiLSTM(self, paragraph, question, mask_paragraph, mask_question, dropout_rate):#, encoder_state_input, scope="", reuse=False):

        with vs.variable_scope("QuestionParagraph", reuse=None):
            questionLen = tf.reduce_sum(tf.cast(mask_question, tf.int32), axis=1)
            (hiddenStatesFw, hiddenStatesBw), (lastStatesFw, lastStatesBw) = tf.nn.bidirectional_dynamic_rnn(self.cell, self.cell, inputs=question,
                                                                        sequence_length=questionLen, dtype=tf.float32)

            lastStateFwQuestion = lastStatesFw[1]
            lastStateBwQuestion = lastStatesBw[1]

        with vs.variable_scope("QuestionParagraph", reuse=True):
    ...
    def encodeLinear(self, paragraph, max_length_paragraph, dropout_rate):
        """
        ...
        with tf.variable_scope("LogisticRegression"):
            for time_step in range(paragraph.get_shape()[1]):
                ...
                pred_s.append(tf.nn.dropout(tf.matmul(paragraph[:,time_step,:], W_s), dropout_rate) + b_s)
                pred_e.append(tf.nn.dropout(tf.matmul(paragraph[:,time_step,:], W_e), dropout_rate) + b_e)
    ...
    def encode(self, paragraph, question, mask_paragraph, mask_question, dropout_rate, encoder_state_input, max_length_paragraph, n_hidden):
        ...
        return(self.BiLSTM(paragraph, question, mask_paragraph, mask_question, dropout_rate))
...
class QASystem(object):
    ...
    def setup_embeddings(self):
        ...
        with vs.variable_scope("embeddings"):
            ...
            self.embeddings = tf.constant(embedds["glove"], dtype=tf.float32)
